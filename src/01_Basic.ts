...
const sum = (a: number, b: string) => {  // 숫자와 문자열을 인수로 받는 함수 선언
  return a + Number(b);  // 인수를 숫자로 변환하여 합산한 값을 반환
};

let postId: string | number;  // 문자열 또는 숫자 값을 저장할 수 있는 변수 선언
let isActive: number | boolean | string;  // 숫자, 불리언, 문자열 값을 저장할 수 있는 변수 선언

let re: RegExp = /\w+/g;  // 정규 표현식
